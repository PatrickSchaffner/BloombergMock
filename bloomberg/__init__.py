...
for pkg in root_dir.iterdir():
    ...
    try:
        ...
        sys.modules[f'bloomberg.{pkg.name}'] = impl  # bloomberg.xbbg = xbbg
...
if len(import_errors) > 0:
    ...
    for pkg in import_errors.keys():
        try:
            for mod in _walk_pkg_dir(base=pkg, pkg_dir=root_dir / pkg):  # Import top-level and nested packages/modules.
                impl = import_module(mod)  # import xbbg.blp -> sys.path entry matches.
                sys.modules[f"bloomberg.{mod}"] = impl  # bloomberg.xbbg.blp = xbbg.blp (prevent copy of mock package)
        except ImportError as e:
